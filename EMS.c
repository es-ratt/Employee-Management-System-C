...
int readEmployeeBlock(FILE* fp, struct Employee* emp) {
    ...
    if (fgets(line, LINE_LEN, fp) == NULL) return 0;
    if (sscanf(line, "ID: %d", &emp->id) != 1) return 0;
...
void updateAttendance() {
    ...
    if (found) printf("Attendance updated successfully.\n");
    else printf("Employee with ID %d not found.\n", id);
}
...
void hrMenu() {
    ...
    do {
        ...
        switch (choice) {
            ...
            case 4: return; // switch role
            case 5: printf("Exiting HR panel.\n"); exit(0);
            default: printf("Invalid option!\n");
        }
    } while (1);
}
